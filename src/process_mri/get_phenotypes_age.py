...
a1_df = a1_df[['SITE_ID','subject','AGE_AT_SCAN']]
a1_df.rename(columns={'SITE_ID':'site_id', 'subject':'participant_id', 'AGE_AT_SCAN':'age'}, inplace=True)
...
a2_df = a2_df[['site_id','participant_id','age_at_scan ']]
...
a2_df.rename(columns={'age_at_scan ':'age'}, inplace=True)
...
for fpath in on_files:
    ...
    if 'age' not in foo.columns:
        print(fpath)
        continue
    on_df = on_df.append(foo[['site_id', 'participant_id','age']])
...
for fpath in adhd_files:
    ...
    foo = foo[['site_id', 'ScanDir ID', 'Age']]
    foo.rename(columns={'ScanDir ID':'participant_id','Age':'age'}, inplace=True)
...
bool_vec = bar
# save
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_img.npy"), img[bool_vec])
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_age.npy"), age[bool_vec])
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_site.npy"), site[bool_vec])
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_ID.npy"), part_id[bool_vec])
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_site_ID.npy"), site_part_id[bool_vec])
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_study.npy"), study[bool_vec])
...
foo = np.load(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-processed_filteredage_site_ID.npy"), allow_pickle=True)
bar = [np.where(x == site_part_id)[0][0] for x in foo]
bar = np.array(bar)
img = img[bar]
np.save(os.path.join(sdata, "neuroimaging/processed/numpy/2019-07-22-unprocessed_filteredage_img.npy"), img)
